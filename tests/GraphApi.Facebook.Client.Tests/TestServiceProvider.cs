...
using GraphApi.Facebook.Client.Tests.Config;
...
namespace GraphApi.Facebook.Client.Tests
{
    ...
    public class TestServiceProvider
    {
        ...
        public static async Task InitTestLibrary(TestContext testContext)
        {
            ...
            var sectionModel = new TestUserIdConfig();
            config.GetSection("Facebook").Bind(sectionModel);

            var testUserApi = ServiceProvider.GetService<ITestUserFbService>();
            TestUserAccessToken = (await testUserApi.GetTestUsers()).First(testUser => testUser.Id == sectionModel.TestUserId).AccessToken;
